...
from typing import Any, Generic, TypeVar, Dict, Callable as TypingCallable
...
class Agent(Generic[Context]):
	...
	async def step(self, step_info: AgentStepInfo | None = None) -> None:
		...
		try:
			...
			if len(result) > 0 and result[-1].is_done:
				...
				self.final_llm_message_from_result = result[-1].extracted_content
	...
	async def run(
	...
	) -> Dict[str, Any]:  # MODIFICADO
		"""Execute the task with maximum number of steps"""

		print("<<<<< USANDO MI VERSIÓN MODIFICADA DE BROWSER_USE AGENT.RUN() >>>>>")
		...
		final_llm_message_content: str | None = None
		...
		try:
			...
			for step_num in range(max_steps):
				...
				if self.state.history.is_done():
					if self.settings.validate_output and step_num < max_steps - 1:
						if not await self._validate_output():
							continue

					final_action_result_obj = self.state.history.final_result()
					if final_action_result_obj and hasattr(final_action_result_obj,
														   'extracted_content') and final_action_result_obj.extracted_content:
						final_llm_message_content = final_action_result_obj.extracted_content
					await self.log_completion()
					break
			else:
				...
				self.state.history.history.append(
					AgentHistory(
						...
						state=BrowserStateHistory(url='', title='', tabs=[], interacted_element=[], screenshot=None, ),
						metadata=None,
					)
				)
				logger.info(f'❌ {agent_run_error}')

			# En lugar de todas las condiciones complicadas, simplemente:
			if hasattr(self, 'final_llm_message_from_result') and self.final_llm_message_from_result:
				final_llm_message_content = self.final_llm_message_from_result
			else:
				# Intentar obtener del último resultado
				final_result = self.state.history.final_result()
				if final_result and hasattr(final_result, 'extracted_content') and final_result.extracted_content:
					final_llm_message_content = final_result.extracted_content
				else:
					final_llm_message_content = "Task completed but no message extracted"

			# Log para debug
			logger.info(
				f"=== FINAL LLM MESSAGE TO RETURN: {final_llm_message_content[:200] if final_llm_message_content else 'None'}...")

			return {
				"history": self.state.history,
				"final_llm_message": final_llm_message_content
			}

		except KeyboardInterrupt:
			logger.info('Got KeyboardInterrupt during execution, returning current history')
			# agent_run_error = 'KeyboardInterrupt' # ya no se usa directamente aquí
			final_action_result_obj = self.state.history.final_result()
			if final_action_result_obj and hasattr(final_action_result_obj,
												   'extracted_content') and final_action_result_obj.extracted_content:
				final_llm_message_content = final_action_result_obj.extracted_content
			else:
				final_llm_message_content = "Task interrupted by user."
			return {
				"history": self.state.history,
				"final_llm_message": final_llm_message_content
			}
			...
			# agent_run_error = str(e) # ya no se usa directamente aquí
			final_llm_message_content = f"Task failed with exception: {str(e)}"
			return {
				"history": self.state.history,
				"final_llm_message": final_llm_message_content
			}
			...
			if not self._force_exit_telemetry_logged:
				try:
					self._log_agent_event(max_steps=max_steps,
										  agent_run_error=agent_run_error)  # agent_run_error se define en el try
